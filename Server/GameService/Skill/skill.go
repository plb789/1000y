...
type SkillBase struct {
	...
	WeaponType  uint8  `gorm:"column:weapon_type" json:"weapon_type"`            // 武器类型限制(0=无限制)
	// 战斗属性
	Damage      int `gorm:"column:damage" json:"damage"`             // 基础伤害
	MpCost      int `gorm:"column:mp_cost" json:"mp_cost"`           // 内力消耗
	Cooldown    int `gorm:"column:cooldown" json:"cooldown"`         // 冷却时间(秒)，0=无冷却走攻速
	Range       int `gorm:"column:range" json:"range"`               // 攻击范围(1=近战, 2-5=远程)
	CastTime    int `gorm:"column:cast_time" json:"cast_time"`       // 施法时间(毫秒)
	AoeRadius   int `gorm:"column:aoe_radius" json:"aoe_radius"`     // AOE范围半径(0=单体)
	AttackSpeed int `gorm:"column:attack_speed" json:"attack_speed"` // 攻速值(1-100，越低越快，0=不适用)
}
